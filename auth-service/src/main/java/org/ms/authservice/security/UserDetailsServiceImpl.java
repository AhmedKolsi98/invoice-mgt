    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        List<SimpleGrantedAuthority> authorities = user.getRoles().stream()
                .flatMap(role -> {
                    ...
                    List<SimpleGrantedAuthority> auths = List.of(new SimpleGrantedAuthority("ROLE_" + role.getName().name()));
                    // Add permissions as authorities
                    if (role.getPermissions() != null) {
                        auths = new java.util.ArrayList<>(auths);
                        auths.addAll(role.getPermissions().stream()
                                .map(permission -> new SimpleGrantedAuthority(permission.name().toLowerCase().replace('_', ':')))
